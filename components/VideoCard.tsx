...
export function VideoCard({ video, compact = false }: { video: Video; compact?: boolean }) {
  return (
    <a
      ...
      target="_blank"
      rel="noreferrer"
      className="group block overflow-hidden rounded-lg bg-white shadow-card ring-1 ring-slate-100 transition hover:-translate-y-0.5 hover:shadow-lg dark:bg-slate-900 dark:ring-slate-800"
    >
      <article>
        ...
        <div className={compact ? "p-3" : "p-4"}>
          <h3 className="line-clamp-2 text-sm font-black leading-6 text-gangwon-navy dark:text-white sm:text-base">{video.title}</h3>
